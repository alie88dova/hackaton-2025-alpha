...
def generate_offer(predicted_income, features):
    ...
    if debt_other > 100000:
        ...
        offer_list.append({
            ...
            "description": f"Мы видим долг {int(debt_other):,} ₽ в других банках. Переведите его в Альфу под сниженный процент.",
            "highlight": "Выгодно",
            "rate": 12.5,
            "termMonths": 60,
            "amountFrom": 50000,
    ...
    if predicted_income > 150000 and (official_salary < 30000 or pd.isna(official_salary)):
        offer_list.append( {
            ...
            "description": "Легализуйте свой доход. Откроем счет за 0 ₽, бухгалтерия в подарок, личные переводы без комиссии.",
            "highlight": "Для профи",
            "rate": 0.0,
            "termMonths": 12,
            "amountFrom": 0,
    ...
    if travel_spend > 5000 or predicted_income > 200000:
        ...
        offer_list.append({
            ...
            "description": "Лучшая карта для поездок. До 10% милями за авиабилеты и отели, бесплатная страховка за рубежом.",
    ...
    if predicted_income < 60000:
        ...
        offer_list.append( {
            ...
            "description": "Идеально для старта. Покупайте сейчас, платите потом. 365 дней без процентов на всё.",
    ...
    return offer_list if offer_list != [] else [{
        ...
        "description": "Нужен только паспорт. Деньги поступят на карту моментально после подписания в приложении.",
        "highlight": "Быстро",
        "rate": 15.9,
        "termMonths": 60,
        "amountFrom": 50000,
